...
- 🌗 **桌面直显（Bare）**：低亮度窗口直透桌面，直接采样悬浮窗覆盖的整块背景明暗自动着色（截断中段平均抗噪 + 事件驱动 + 10s 低频兜底），浅/深/纯色背景均可靠切换。
...
- 修复桌面直显深背景看不清：亮度采样改取悬浮窗覆盖的整块桌面（Bare 窗口透明故即背景本身），用截断中段平均抗字体/图标噪点，并增加 10s 低频兜底采样，背景被其他窗口遮挡（无拖动事件）也能及时切换文字颜色，浅/深/纯色背景下均按主体背景明暗准确切换
